...
ap.add_argument('--timestamp', action='store', default='%Y-%m-%d %H:%M:%S', metavar='FORMAT', help='Controls format of date, added, and viewed fields. Understands strftime directives. Defaults to \'%%Y-%%m-%%d %%H:%%M:%%S\' (eg, \'' + datetime.datetime.now().strftime('%Y-%m-%d %H:%M:%S') + '\').')
...
ap.add_argument('--show', action='store', default='all', choices=['unread', 'read', 'all'], metavar='FILTER', help='Control which items to output. Acceptable FILTER values are unread, read, or all. Defaults to all.')
